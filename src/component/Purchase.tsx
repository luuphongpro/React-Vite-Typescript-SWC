...
const Purchase = () => {
    ...
    return (
        <div className="bg-gray-200 dark:bg-gray-800">
            <div className="lg:w-4/6 mx-auto bg-white dark:bg-gray-700 lg:p-5">
                <h1 className="text-xl font-bold py-2">PURCHASE HISTORY</h1>
                {listPurchare.length === 0 ? (
                    <p>You haven't made a purchase yet.</p>
                ) : (
                    <>
                        <div className="flex flex-row h-10 items-center justify-end gap-10 lg:gap-50 border-y border-gray-500 dark:bg-gray-400 my-5">
                        ...
                        {listPurchare.map((items: any, index: number) => (
                            <>
                                <div className="mt-10 dark:bg-gray-400 p-5">
                                    ...
                                    <div className="font-bold">Order Date: <span className="font-medium">14/06/2025</span></div>
                                </div>
                                {items.map((item: Itemcart) => (
                                    <>

                                        <div
                                            className="flex flex-row items-center justify-between  gap-2 border-y border-gray-500 lg:my-5 dark:bg-gray-400 rounded-l-lg"
                                            key={item.id}
                                        >
                                            ...
                                            <div className="flex items-center justify-between">
                                                ...
                                                <div className="mx-10 text-red-600 font-bold lg:w-15">${((item.price - (item.price * item.discountPercentage / 100)) * item.quantity).toFixed(2)}</div>
